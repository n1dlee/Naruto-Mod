...
1.8.1 down was generated by release-please against the upstream repository, which this fork
no longer follows.

### Features

* **Tailed beasts.** All eight, One Tail through Eight, with their imported models, their own
  roars and the Tailed Beast Bomb. Each tail doubles the beast: Kurama sits at 256x Shukaku,
  and health and damage are derived from that ladder rather than typed in per beast.
* **Kurama.** The real Nine Tails model, nine tails and all, with a travelling-wave tail
  animation. Used by both the player's Full Avatar and the Naruto boss's final stage.
* **Named summons.** Gamabunta, Katsuyu, Manda and Aoda, with a fifth element and mobs that
  walk on water.
* **Sasori's puppet corps.** Five puppets including Hiruko, fought alongside their master
  rather than replaced by a handful of senbon.
* **Gaara and the sand.** A shield that eats hits outright, and Shukaku at the end of it.
* **Eleven more wielders**, plus Temari's fan.
* **Chibaku Tensei.** A real entity: a 128-block pull that excavates the ground, gathers it
  into a moon and lands like a meteor.
* **Haku's Ice Mirrors.** Eight destructible panes as entities.
* **Chakra sight.** Byakugan, Sharingan, Senjutsu and Kurama Chakra Mode all drive the vision
  overlay, at their own ranges - Senjutsu reaches furthest, which is how Naruto found Nagato.
* **Boss phases** are each a full, larger health bar, so a boss's most interesting form is not
  the one you get two swings of.
* **Wall running** got its own pose, taking priority over the sprint animation.

### Bug Fixes

* Fireball could be cast for free; its cooldown and Wind Palm's were both wrong.
* Three validation holes in the ability packet.
* Boss chakra was not a real constraint, and boss AoE hit its own allies.
* Bijudama could not be interrupted once started.
* The Naruto boss's shadow clones did not wear his skin.
* Anvil crash on armour repair; Katsuyu rendered untextured; substitution regeneration
  divided by zero.
* Model converter silently dropped whole subtrees when an imported model's own field name
  collided with an inherited biped part.
