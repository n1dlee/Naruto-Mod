...
public class NarutoKeyHandler {
    ...
    private static boolean wallWalkJumpConsumed = false;
    ...
    public static void registerKeyBinds(RegisterKeyMappingsEvent event) {
        ...
        CHIDORI_KEY.registerClickConsumer( () -> {
            Minecraft mc = Minecraft.getInstance();
            if(mc.player != null ) {
    ...
    private static void handleWallWalkJumpDetach() {
        ...
        if (wallWalkJumpConsumed) {
            return;
        }
        mc.player.getCapability(NinjaCapabilityHandler.NINJA_DATA).ifPresent(ninjaData -> {
            if (ninjaData.isWallWalkAttached()) {
                ninjaData.setWallWalkDetachTicks(14);
                PacketHandler.sendToServer(new ServerWallWalkDetachPacket());
                wallWalkJumpConsumed = true;
            }
        });
    }
